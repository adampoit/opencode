...
if $merge_had_conflicts; then
  ...
  if [[ -n "$unmerged_files" ]]; then
    ...
    log "Preparing conflicted sync branch from ${release_tag}"
...
trap 'rm -f "$body_file"' EXIT
{
  ...
  if $conflicted; then
    printf '\n## Conflicts\n'
    printf 'Git could not merge `%s` into `%s` automatically, so this branch points at the upstream release with fork policy applied. Resolve the PR conflicts before merging.\n\n' "$release_tag" "$BASE_BRANCH"
